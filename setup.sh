...
print_quickstart() {
  ...
  echo "  /proveit I want to build a habit tracker for remote teams"
  echo ""
  ...
  echo "  - Automated market research and competitor analysis (research-N.md)"
  echo "  - Optional deep-dive Research Swarm (swarm-N-synthesis.md)"
  echo "  - Confidence scoring with honest kill signals"
  echo "  - discovery.md — index of scores, findings, and file references"
  echo "  - Gamma presentation for technical handoff"
  echo ""
  info "  Tip: Create a directory for each idea:"
  echo "  mkdir ~/my-idea && cd ~/my-idea && /proveit"
  echo ""
}
