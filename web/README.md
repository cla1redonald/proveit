...
| `NEXT_PUBLIC_POSTHOG_KEY` | Optional | PostHog project key for browser analytics. It starts only after a visitor opts in through the consent banner. | [posthog.com](https://posthog.com) project settings |
| `NEXT_PUBLIC_POSTHOG_HOST` | Optional | PostHog ingestion host. Defaults to `https://eu.i.posthog.com`. | — |
| `POSTHOG_SERVER_ENABLED` | Optional | Set to `true` only if server-side PostHog error/event telemetry has been reviewed and is wanted. Defaults to off. | — |
...
- [ ] **Privacy notice and analytics consent** reviewed against the deployed data flows
...
**Analytics and error tracking via PostHog.** Browser analytics, autocapture, page tracking, and client exception capture start only after the visitor opts in through the consent banner. Server-side errors and business events are disabled unless `POSTHOG_SERVER_ENABLED=true` is explicitly set. **PII discipline:** the user's idea text, chat content, and email addresses are not intentionally passed to PostHog; captures are restricted to route metadata, error type, and other non-content context. See the public [privacy notice](/privacy) for the data flow and consent controls. Source maps are uploaded automatically when the PostHog build credentials are present; maps are deleted from `.next/static` after upload.
