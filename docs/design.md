...
- **3.8.0 (2026-08-02)** — Agent maintainability + shared-types audit. `agents/proveit.md` slimmed to an orchestrator (~160 lines) with phase content moved verbatim into `agents/phases/` (00-intake through 08-outputs, plus `09-scoring.md` and `fast-mode.md`), output templates in `agents/templates/output-formats.md`, and swarm Task-tool fallback prompts referenced in `agents/swarm/agent-prompts.md` (canonical prompts remain in `scripts/swarm.workflow.mjs`). Web spend-ledger calibration logging added (`[spend-calibration]` structured logs when Anthropic reports token usage). Shared-types audit documented: web `Verdict` ⊆ core `FastCheckVerdict`; `KillSignal` intentionally differs (runtime chat vs vault parse). Parity test: `packages/core/tests/types-parity.test.ts`. Spec: `docs/specs/2026-08-02-spend-calibration.md`.
