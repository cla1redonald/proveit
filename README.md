...
- **Automated (Managed Agent, daily at 05:00 UTC):** Deploy the `frontier-scan` Managed Agent for hands-off daily refreshes. See [`docs/managed-agent-setup.md`](docs/managed-agent-setup.md) for deployment instructions. The agent runs independently on Anthropic's infrastructure, commits snapshot refreshes, and PRs any agent-impacting changes for your review.
