...
**How to run it:** in a Claude Code session, ask Claude to run the `frontier-scan` workflow. It runs on your Max subscription (no API key, no extra cost), refreshes the snapshot, and flags any agent-impacting shifts for you to apply. Do it every couple of weeks, or whenever a big model drops. `args.mode` is `full` (deep, ~100 agents) or `lite` (a quicker, lighter pass). There is deliberately no scheduled/CI automation — an unattended cron bills the Anthropic API (separate from your Max plan), so this stays a manual, subscription-funded action you trigger when you want it.
