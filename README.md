...
│   ├── deploy-frontier-agent.ts    # Deploy frontier-scan Managed Agent to Anthropic infrastructure
...
- **Automated (Managed Agent, every 2 weeks):** The `frontier-scan` Managed Agent runs automatically on Anthropic's infrastructure (every 1st & 15th at 05:00 UTC). It checks for model announcements, flags [AGENT-IMPACT] changes, and auto-PRs updates. See [`docs/managed-agent-setup.md`](docs/managed-agent-setup.md) for status, monitoring, and how to re-deploy. **Current agent ID:** `agent_01FoF2hJGMYJ5cP6P9v1EbVe`
