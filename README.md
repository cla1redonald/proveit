...
proveit/
├── agents/
│   ├── proveit.md              # The main agent definition (Opus model)
│   └── frontier-scan-agent.md  # Managed Agent for daily frontier refreshes (Sonnet)
...
│   ├── managed-agent-setup.md  # Deploy frontier-scan as a scheduled Managed Agent
...
│   ├── deploy-frontier-agent.ts    # Creates/updates the frontier-scan Managed Agent object (NOT the schedule)
...
- **Manual (on your Max subscription):** In a Claude Code session, ask Claude to run the `frontier-scan` workflow. It runs on your Max subscription (no API key, no extra cost), refreshes the snapshot, and flags any agent-impacting shifts for you to apply. Do it every couple of weeks, or whenever a big model drops. `args.mode` is `full` (deep, ~100 agents) or `lite` (quicker, lower cost).

- **Automated (Managed Agent, every 2 weeks):** A **lightweight** `frontier-scan` Managed Agent (a single Sonnet-5 monitor, **not** the swarm) runs on Anthropic's infrastructure on the 1st & 15th at 05:00 UTC. It web-searches the big-6 labs for model announcements and opens a draft PR **only when** it finds an `[AGENT-IMPACT]` change (silent otherwise). It **bills the API wallet** (separate from Max). See [`docs/managed-agent-setup.md`](docs/managed-agent-setup.md) for full detail. **Agent:** `agent_01FoF2hJGMYJ5cP6P9v1EbVe` · **deployment:** `depl_01Gpjjk7u7H8HJNDzgCDYtL4` (first run 2026-08-15).
