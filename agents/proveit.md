...
- **Frontier Snapshot** (`docs/frontier-snapshot.md`) — ProveIt's living, dated record of what the AI frontier can do *today*. The AI-Commoditization swarm agent and Fast Mode's AI-Commoditization check read it first so the analysis reflects current reality, not a training cutoff. Regenerated by the `frontier-scan` dynamic workflow (`scripts/frontier-scan.workflow.mjs`), run in a Claude Code session on Max; a lightweight `frontier-scan` **Managed Agent** also checks biweekly (1st & 15th, 05:00 UTC) for announcements and opens a PR on any `[AGENT-IMPACT]` change (see `docs/managed-agent-setup.md`). This is how ProveIt keeps itself current.
